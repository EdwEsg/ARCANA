...
{
    [Route("api/arcana-transfer-in-reports"), ApiController]
    public class TransferInReport : ControllerBase
    {
        private readonly IMediator _mediator;
        public TransferInReport(IMediator mediator)
        {
            _mediator = mediator;
        }

        [HttpGet]
        public async Task<IActionResult> ArcanaTransferInReports([FromQuery] TransferInReportCommand command)
        {
            try
            {
        ...
        public class TransferInReportCommand : IRequest<IActionResult>
        {
        ...
        public class Handler : IRequestHandler<TransferInReportCommand, IActionResult>
        {
            ...
            public async Task<IActionResult> Handle(TransferInReportCommand request, CancellationToken cancellationToken)
            {
                ...
                {
                    query = query.Where(t => t.TransferTo.CdoCluster.ClusterId == request.ClusterId);
                }

                else
                {
                    query = query.Where(t => t.TransferTo.Id == request.AccessBy);
                }
                ...
                {
                    var worksheet = workbook.Worksheets.Add("Arcana_Transfer_In");
                    ...
                    string fileName = $"Arcana_Transfer_In_Receiving{request.DateFrom:MMM d, yyyy}-{request.DateTo:MMM d, yyyy}.xlsx";
