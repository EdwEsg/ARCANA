...
{
    [Route("api/arcana-return-report"), ApiController]
    public class ReturnReport : ControllerBase
    {
        private readonly IMediator _mediator;
        public ReturnReport(IMediator mediator)
        {
            _mediator = mediator;
        }

        [HttpGet]
        public async Task<IActionResult> ArcanaReturn([FromQuery] ReturnReportCommand command)
        {
            try
            {
        ...
        public class ReturnReportCommand : IRequest<IActionResult>
        {
        ...
        public class Handler : IRequestHandler<ReturnReportCommand, IActionResult>
        {
            ...
            public async Task<IActionResult> Handle(ReturnReportCommand request, CancellationToken cancellationToken)
            {
                ...
                var finalQuery = query.SelectMany(r => r.ReturnOrderItems)
                    .Select(i => new
                    {
                        ReturnItems = i,
                ...
                {
                    var worksheet = workbook.Worksheets.Add("Arcana_Return");
                    ...
                    string fileName = $"Arcana_Return{request.DateFrom:MMM d, yyyy}-{request.DateTo:MMM d, yyyy}.xlsx";
