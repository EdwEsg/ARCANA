...
{
    [Route("api/arcana-return-by-cdo-reports"), ApiController]
    public class ReturnByCDOReport : ControllerBase
    {
        private readonly IMediator _mediator;
        public ReturnByCDOReport(IMediator mediator)
        {
            _mediator = mediator;
        }

        [HttpGet]
        public async Task<IActionResult> ArcanaMoReports([FromQuery] ReturnByCdoReportCommand command)
        {
            try
            {
        ...
        public class ReturnByCdoReportCommand : IRequest<IActionResult>
        {
        ...
        public class Handler : IRequestHandler<ReturnByCdoReportCommand, IActionResult>
        {
            ...
            public async Task<IActionResult> Handle(ReturnByCdoReportCommand request, CancellationToken cancellationToken)
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    {
                        "Series", "Id", "Receiving Date", "Supplier Code", "Supplier Name", "Item Code", "Item Description", "UOM", "Category", "Product Category", "Quantity", "Slab", "Production Date", "Farm Source", "Description", "Reference", "Reason", "Item Reference", "Account Title", "Company Code", "Company", "Department Code", "Department", "Location Code", "Location", "Account Code", "Account", "Warehouse Code", "Transaction Date", "Encoded By"
                    };
                    ...
                    for (var index = 0; index < consolidate.Count; index++)
                    {
                        ...
                        row.Cell(11).Value = consolidate[index].Quantity;
                        row.Cell(12).Value = "";
                    ...
                    string fileName = $"Arcana_Returned_By_CDO_Receiving{request.DateFrom:MMM d, yyyy}-{request.DateTo:MMM d, yyyy}.xlsx";
